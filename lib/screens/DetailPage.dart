            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (isInitialLoading) {
                    Future.microtask(() => setState(() => isInitialLoading = false));
                  }
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final String albumName = albumData['albumName'] ?? 'Untitled Album';
                      final bool isPhotoInAlbum = photos.contains(widget.postId);
                      ...
                      if (photos.isNotEmpty) {
                        ...
                        return FutureBuilder<DocumentSnapshot>(
                          ...
                          builder: (context, photoSnapshot) {
                            if (photoSnapshot.hasData && photoSnapshot.data!.exists) {
  ...
  onTap: () async {
    if (isPhotoInAlbum) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Photo removed from "$albumName"')),
      );
    } else {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Photo added to "$albumName"')),
      );
    }
  },
);
...
// Fungsi untuk menampilkan dialog pembuatan album baru
...
//                 await _createNewAlbum(albumNameController.text);
...
Future<void> _toggleSaveToAlbum() async {
  try {
    ...
    showAnimatedOverlayNotification(
      context, 
      isSavedToAlbum ? "Saved to album" : "Removed from album"
    );
                                      ...
                                      children: [
                                        ...
                                        FutureBuilder<int>(
                                          future: _getReplyCount(
                                              comment['commentId']),
                                          builder: (context, snapshot) {
                                            int replyCount = snapshot.data ?? 0;
                                            return Row(
                                              children: [
                                                IconButton(
                                                  icon: const Icon(
  ...
  child: ClipRRect(
    ...
    child: widget.imageUrl.isNotEmpty
        ...
        : Shimmer.fromColors(
            ...
            child: AspectRatio(
              aspectRatio: 16 / 9, // Sesuaikan dengan rasio yang diinginkan
              child: Container(
                decoration: BoxDecoration(
                  color: Colors.white,
                  borderRadius: BorderRadius.circular(16.0),
